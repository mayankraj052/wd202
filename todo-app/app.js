...
app.delete("/todos/:id", async function (request, response) {
  ...
  try {
    const deleted = await Todo.destroy({
      where: {
        id: request.params.id,  // Delete Todo where ID matches
      },
    });

    // If deletion is successful, the destroy method returns a non-zero value (number of deleted rows).
    response.json(deleted ? true : false);
  } catch (error) {
